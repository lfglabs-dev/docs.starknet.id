...
In this guide, we'll develop a resolver contract leveraging offchain resolving through CCIP Read for subdomains under `notion.stark`, using the Notion API, to resolve subdomains stored in a Notion table.
...
- **No Hints Provided**: When the `resolve`\*function is called without any hints, the contract will trigger a panic, returning an error that signals the need for offchain resolving. This error is an array of short strings. The first element should always be `offchain_resolving` followed by the URL of your api which is predetermined and passed in the constructor of your contract.
...
Deploy your resolver contract with the public key and the URL of your API endpoint. Ensure your API's endpoint is structured to append the domain name directly to the query, such as `https://sepolia.api.ccip-demo.starknet.id/resolve?domain=`. This setup enables the `getAddressFromStarkName` function from `starknetid.js`, used later in the frontend, to function seamlessly.
...
```bash
LibraryError: RPC: starknet_call with params {"request":{"contract_address":"0x0707f09bc576bd7cfee59694846291047e965f4184fe13dac62c56759b3b6fa7","entry_point_selector":"0x2e269d930f6d7ab92b15ce8ff9f5e63709391617e3465fff79ba6baf278ce60","calldata":["0x2","0xf41de","0x3f29fbcd","0x0"]},"block_id":"pending"}
 40: Contract error: {"revert_error":"Error in the called contract (0x0707f09bc576bd7cfee59694846291047e965f4184fe13dac62c56759b3b6fa7):\nError at pc=0:16405:\nGot an exception while executing a hint: Execution failed. Failure reason: (0x6f6666636861696e5f7265736f6c76696e67 ('offchain_resolving'), 0x68747470733a2f2f7365706f6c69612e6170692e636369702d64656d6f2e73 ('https://sepolia.api.ccip-demo.s'), 0x7461726b6e65742e69642f7265736f6c76653f646f6d61696e3d ('tarknet.id/resolve?domain=')).\nCairo traceback (most recent call last):\nUnknown location (pc=0:304)\nUnknown location (pc=0:5803)\nUnknown location (pc=0:11494)\n\nError in the called contract (0x0153be68cf8fc71138610811dd2b4fa481eb99f3eedcb3fce7369569055be275):\nExecution failed. Failure reason: (0x6f6666636861696e5f7265736f6c76696e67 ('offchain_resolving'), 0x68747470733a2f2f7365706f6c69612e6170692e636369702d64656d6f2e73 ('https://sepolia.api.ccip-demo.s'), 0x7461726b6e65742e69642f7265736f6c76653f646f6d61696e3d ('tarknet.id/resolve?domain=')).\n"}
    at RpcProvider.errorHandler (index.mjs:2753:13)
    at RpcProvider.fetchEndpoint (index.mjs:2769:12)
    at async RpcProvider.callContract (index.mjs:3144:20)
    at async tryResolveDomainSID (resolvingForm.tsx:85:25)
    at async getAddressFromStarkName (resolvingForm.tsx:45:14)
    at async submit (resolvingForm.tsx:33:22)
```
