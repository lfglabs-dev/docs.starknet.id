...
```tsx
const address = starknetIdNavigator.getAddressFromStarkName("test.stark");
```
...
```tsx
const domain = starknetIdNavigator.getStarkName(
...
```tsx
const domain = starknetIdNavigator.getStarknetId("fricoben.stark");
```
...
```tsx
const domain = starknetIdNavigator.getVerifierData("fricoben.stark", "discord");
```
...
```tsx
const domain = starknetIdNavigator.getExtendedVerifierData(
  "fricoben.stark",
  "discord",
  1
);
...
Decode starknetid domain represented as an array of bigint `[454245]` -> `test.stark`
...
| Name      | Type       | Description              |
| :-------- | :--------- | :----------------------- |
| `encoded` | `BigInt`[] | Encoded domain to decode |
...
A `BigInt[]` representing the domain encoded.
